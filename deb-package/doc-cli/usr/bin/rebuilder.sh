...
function build(){
    ...
    if [[ "$trimmed_last_commit" == "$trimmed_commit" ]]; then
        ...
        updateCommit "$1"
        cd $1
        command docker compose build --build-arg GIT="$GIT" >/var/log/doc_cli/build.log
        command docker compose up -d >/var/log/doc_cli/build.log
        echo "$NAME - rebuilded sucessfully - $(date +'%d/%m/%Y %H:%M:%S')"
    fi
}
